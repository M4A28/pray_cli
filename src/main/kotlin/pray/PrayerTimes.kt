...
import pray.data.CalendarUtil
import pray.data.DateComponents
import pray.data.TimeComponents
import pray.internal.Coordinates
import pray.internal.SolarTime
import java.util.*
...
class PrayerTimes(
    ...
    init {
        ...
        if (!error) {
            ...
            if (calculationParameters.ishaInterval > 0) {
                tempIsha = CalendarUtil.add(tempMaghrib, calculationParameters.ishaInterval * 60, Calendar.SECOND)
            } else {
                timeComponents = TimeComponents.fromDouble(
                ...
                if (calculationParameters.method == CalculationMethod.MOON_SIGHTING_COMMITTEE &&
                    ...
                    tempIsha = CalendarUtil.add(sunsetComponents, nightFraction.toInt(), Calendar.SECOND)
                }
                val safeIsha: Date
                safeIsha = if (calculationParameters.method == CalculationMethod.MOON_SIGHTING_COMMITTEE) {
        ...
        if (error || tempAsr == null) {
            ...
            fajr = CalendarUtil.roundedMinute(
                CalendarUtil.add(
                    CalendarUtil.add(tempFajr, calculationParameters!!.adjustments.fajr, Calendar.MINUTE),
            ...
            sunrise = CalendarUtil.roundedMinute(
                CalendarUtil.add(
                    CalendarUtil.add(tempSunrise, calculationParameters.adjustments.sunrise, Calendar.MINUTE),
            ...
            dhuhr = CalendarUtil.roundedMinute(
                CalendarUtil.add(
                    CalendarUtil.add(tempDhuhr, calculationParameters.adjustments.dhuhr, Calendar.MINUTE),
            ...
            asr = CalendarUtil.roundedMinute(
                CalendarUtil.add(
                    CalendarUtil.add(tempAsr, calculationParameters.adjustments.asr, Calendar.MINUTE),
            ...
            maghrib = CalendarUtil.roundedMinute(
                CalendarUtil.add(
                    CalendarUtil.add(tempMaghrib, calculationParameters.adjustments.maghrib, Calendar.MINUTE),
            ...
            isha = CalendarUtil.roundedMinute(
                CalendarUtil.add(
                    CalendarUtil.add(tempIsha, calculationParameters.adjustments.isha, Calendar.MINUTE),
    ...
    fun timeForPrayer(prayer: Prayer?): Date {
        return when (prayer) {
            ...
            Prayer.ISHA -> isha!!
            else -> Date(fajr!!.time + 1000 * 24 * 60 * 60L)
        }
    }
    ...
    companion object {
        private fun seasonAdjustedMorningTwilight(
            ...
            return CalendarUtil.add(sunrise, -Math.round(adjustment * 60.0).toInt(), Calendar.SECOND)
        }
